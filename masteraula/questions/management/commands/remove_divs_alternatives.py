...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for discipline in options['discipline']:
            ...
            questions =[q.id for q in d.question_set.filter().order_by('id')]
            alternatives = Alternative.objects.filter(question__in=questions).filter(text__contains='<div').order_by('id')
            statements = [(q.text, q.id) for q in alternatives]
